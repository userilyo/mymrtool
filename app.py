...
# Define more efficient regular expressions for different PII detection and anonymization use cases
regex_patterns = {
    "uk_ss_id": re.compile(r'\b[A-Z]{2}\s\d{6}\s[A-Z]\b'),  # e.g. AB 123456 A
    "uk_phone_number": re.compile(r'^(?:\+44\d{10}|0\d{10})$'),
    "uk_postcode": re.compile(r'\b[A-Z]{1,2}\d{1,2}[A-Z]?\s*\d[A-Z]{2}\b', re.IGNORECASE),
